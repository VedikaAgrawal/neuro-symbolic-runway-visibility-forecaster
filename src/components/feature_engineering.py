...
class FeatureEngineering:
    ...
    def run_feature_engineering(self):
        ...
        """
        logger.info(f"🛠️ [FeatureEngineering] Ingesting master fused dataset from: {self.input_file}")
        if not os.path.exists(self.input_file):
            logger.error(f"[FeatureEngineering] Master fused file not found at path: {self.input_file}")
        ...
        try:
            ...
            logger.info(f"[FeatureEngineering] Dropped {initial_len - len(df)} rows with missing target. Count is now: {len(df)}")
            ...
            logger.info("[FeatureEngineering] Interpolating urban and rural spatial gaps...")
            df[interpolate_cols] = df[interpolate_cols].interpolate(method='linear', limit_direction='both')
            
            # 4. Domain Feature Calculations
            logger.info("[FeatureEngineering] Extrapolating complex meteorological domain attributes...")
            ...
            if missing_features:
                logger.error(f"[FeatureEngineering] ❌ Verification failed. Required feature columns are missing from the engineered frame: {missing_features}")
                raise ValueError(f"🚨 Features missing after engineering stage: {missing_features}")
                
            nan_counts = df[FEATURE_COLS].isna().sum().sum()
            if nan_counts > 0:
                logger.warning(f"[FeatureEngineering] ⚠️ Caution: {nan_counts} NaN cells detected after interpolation/ffill. Forcing constant values to 0.0...")
                df[FEATURE_COLS] = df[FEATURE_COLS].fillna(0.0)
                
            # Export processed file
            os.makedirs(os.path.dirname(self.output_file), exist_ok=True)
            df.to_csv(self.output_file, index=False)
            logger.info(f"[FeatureEngineering] ✅ Stage completed! Exported engineered dataset to: {self.output_file}")
            
            # 6. Upload to S3 if online
            logger.info("Attempting engineered dataset backup to S3 storage...")
            upload_to_s3(self.output_file, "data/delhi_2024_engineered.csv")
            
            return self.output_file
        except Exception as fe_err:
            logger.error(f"❌ [FeatureEngineering] Engineering calculations/exports failed: {fe_err}", exc_info=True)
            raise fe_err
