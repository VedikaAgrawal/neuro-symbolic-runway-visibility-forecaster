...
class DataValidation:
    ...
    def validate_dataset(self, file_path):
        ...
        """
        logger.info(f"🔍 [DataValidation] Commencing dataset validation diagnostics on: {file_path}")
        if not os.path.exists(file_path):
            logger.error(f"[DataValidation] Dataset file not found at path: {file_path}")
        ...
        try:
            ...
            if missing_cols:
                ...
                logger.error(f"[DataValidation] ❌ Structural schema validation failed! Missing mandatory columns: {missing_cols}")
                return False, report
            ...
            if len(violations) > 0:
                ...
                logger.warning(f"[DataValidation] ⚠️ Meteorological range anomalies/alerts detected:\n" + "\n".join(violations))
            ...
            logger.info("[DataValidation] ✅ Dataset structure successfully validated! Schema is perfectly aligned.")
            return True, report
        except Exception as val_err:
            logger.error(f"❌ [DataValidation] Fatal error occurred during validation process: {val_err}", exc_info=True)
            return False, {"error": str(val_err)}
